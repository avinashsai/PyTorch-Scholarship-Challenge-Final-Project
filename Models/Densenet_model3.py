...
def pretrainedmodel():
  ...
  final_features = model.classifier.in_features
  ...
  model_seq = nn.Sequential(
      ...
      nn.ReLU(),
      nn.Linear(layer1,out),
      nn.LogSoftmax(dim=1)
  )
  
  model.classifier = model_seq
...
torch.save(model_conv.state_dict(),'densenet201_3.pt')
...
model_conv_.load_state_dict(torch.load('densenet201_3.pt'))
model_conv_ = model_conv_.to(device)


test_accuracy,test_loss = validation_accuracy(model_conv,dataloaderstest['test'])


print("Test Accuracy for Google Dataset {}".format(test_accuracy))
...
test_accuracyorg,test_lossorg = validation_accuracy(model_conv,dataloadersorg['testoriginal'])

print("Test Accuracy for Flowers Dataset {}".format(test_accuracyorg))